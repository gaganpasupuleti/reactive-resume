...
export function ResumeAnalysisSectionBuilder() {
	...
	const { mutate: analyzeResume, isPending } = useMutation({
		...orpc.ats.analyzeResume.mutationOptions(),
		onSuccess: (analysis) => {
		...
		onError: (error) => {
			...
			toast.error(t`Failed to analyze resume.`, { description: message });
		},
	});
	...
	const canSubmit = jdText.trim().length > 0 && !isPending;
					...
					<div className="grid grid-cols-2 items-center gap-3">
						<div>
							<p className="text-muted-foreground text-xs">
								<Trans>Get a review of your resume with an overall score, strengths, and actionable suggestions.</Trans>
							</p>
						</div>

						<Button disabled={isPending} onClick={onOpenAnalyzeDialog} className="ml-auto w-fit">
							<SparkleIcon />
							{analyzeLabel}
						</Button>
					</div>
			...
			<Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
				<DialogContent className="gap-5 sm:max-w-lg">
					<DialogHeader className="pe-8">
						...
						<DialogDescription>
							<Trans>Paste the job description to compare this resume against the role.</Trans>
						</DialogDescription>
					</DialogHeader>

					<div className="space-y-2">
						<Label htmlFor="resume-analysis-jd">
							<Trans>Job description</Trans>
						</Label>
						<Textarea
							id="resume-analysis-jd"
							value={jdText}
					...
					<DialogFooter>
						...
						<Button onClick={onSubmitAnalysis} disabled={!canSubmit}>
							{isPending ? <Trans>Analyzing…</Trans> : <Trans>Analyze</Trans>}
						</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
